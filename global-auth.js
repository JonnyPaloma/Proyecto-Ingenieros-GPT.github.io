...
const SAFE_RETURN_PAGES = new Set(['chat.html', 'index.html', 'panel-usuario.html']);
...
function dashboardForRole(role) {
  return role === 'Admin' || role === 'Employee' ? 'panel-admin.html' : 'panel-usuario.html';
}
...
function requestedReturnPage() {
  ...
  const normalized = value.split(/[?#]/)[0].split('/').pop()?.toLowerCase();
  return SAFE_RETURN_PAGES.has(normalized) ? normalized : null;
}
...
async function redirectToDashboard(session) {
  ...
  const returnPage = requestedReturnPage();
  const destination = returnPage === 'panel-usuario.html' && role !== 'User'
    ? dashboardForRole(role)
    : returnPage || dashboardForRole(role);
...
window.EligeTuVidaAuth = Object.freeze({ redirectToDashboard, getRole, dashboardForRole });
